...
const AddRekening = () => {
  ...
  const getPegawai = async () => {
    try {
      ...
      const [pegawaiResponse, rekeningResponse] = await Promise.all([
        axios.get(`${apiUrl}/pegawai`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get(`${apiUrl}/rekening`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);

      const existingIds = new Set(
        (rekeningResponse.data || []).map((item) => item.idPegawai),
      );
      ...
      <div className="relative z-10 p-7">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4 mb-8">
          <div className="flex-1">
            ...
            >
              Formulir Penambahan Data Rekening Baru - Kantor Imigrasi Kelas II
              TPI Lhokseumawe
            </p>
          </div>
        </div>
