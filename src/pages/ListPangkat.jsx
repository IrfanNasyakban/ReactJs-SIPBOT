...
import { dummyPangkat } from "../data/dummy";
...
const ListPangkat = () => {
  const [search, setSearch] = useState("");
    ...
    const [pangkat] = useState(dummyPangkat);
            ...
            <div className="flex items-start justify-between flex-wrap gap-4 mb-6">
              <div>
                <h1 className={`text-xl font-bold tracking-wide ${
                  ...
                  Data <span style={{ color: currentColor }}>Pangkat</span>
                </h1>
                <p className="text-xs mt-1" style={{ color: isDark ? "rgba(255,255,255,.35)" : "rgba(0,0,0,.5)" }}>
                  Manajemen data pangkat dan riwayat kenaikan pangkat pegawai
                </p>
              </div>
                  ...
                  <table className="w-full" style={{ minWidth: 1400, borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ background: isDark ? "rgba(56,139,255,.1)" : `rgba(${parseInt(currentColor.slice(1, 3), 16)},${parseInt(currentColor.slice(3, 5), 16)},${parseInt(currentColor.slice(5, 7), 16)},.08)`, borderBottom: `1px solid ${isDark ? "rgba(56,139,255,.2)" : `rgba(${parseInt(currentColor.slice(1, 3), 16)},${parseInt(currentColor.slice(3, 5), 16)},${parseInt(currentColor.slice(5, 7), 16)},.15)`}` }}>
                        {[
                          "No", "Nama", "Pangkat", "Golongan Ruang", "Tanggal SK Pangkat", "Nomor SK Pangkat", "SK Pangkat Dari", "Uraian SK Pangkat", "TMT Pangkat", "Aksi",
                        ].map((h) => (
                          <th
                            key={h}
                      ...
                      {paginated.length === 0 ? (
                        ...
                        paginated.map((p, i) => (
                          ...
                          >
                            ...
                            {/* Tanggal SK Pangkat */}
                            <td className="px-4 py-3 text-sm" style={{ color: isDark ? "rgba(220,235,255,.8)" : "rgba(0,0,0,.7)" }}>
                              {new Date(p.tgl_sk_pangkat).toLocaleDateString('id-ID')}
                            </td>
                            {/* Nomor SK Pangkat */}
                            <td className="px-4 py-3 text-sm font-mono" style={{ color: isDark ? "rgba(180,210,255,.7)" : "rgba(0,0,0,.6)" }}>
                              {p.nomor_sk_pangkat}
                            </td>
                            {/* SK Pangkat Dari */}
                            <td className="px-4 py-3 text-sm" style={{ color: isDark ? "rgba(220,235,255,.8)" : "rgba(0,0,0,.7)" }}>
                            ...
                            {/* TMT Pangkat */}
                            <td className="px-4 py-3 text-sm" style={{ color: isDark ? "rgba(220,235,255,.8)" : "rgba(0,0,0,.7)" }}>
                              {new Date(p.tmt_pangkat).toLocaleDateString('id-ID')}
                            </td>
                            {/* Aksi */}
                            <td className="px-4 py-3">
                              <div className="flex items-center justify-center gap-1.5">
                                {/* View */}
                                <button
                                  onClick={() => navigate(`/pangkat/${p.uuid}`)}
                                  title="Lihat Detail"
                                ...
                                {/* Edit */}
                                <button
                                  onClick={() => navigate(`/pangkat/edit/${p.uuid}`)}
                                  title="Edit"
                                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-150 hover:-translate-y-0.5"
                                  style={{
                                    background: "transparent",
            ...
            >
              ...
              >
                ...
                <p className="text-center text-sm mb-6" style={{ color: isDark ? "rgba(255,255,255,.4)" : "rgba(0,0,0,.5)" }}>
                  Tindakan ini tidak dapat dibatalkan. Data pangkat akan dihapus secara permanen.
                </p>
