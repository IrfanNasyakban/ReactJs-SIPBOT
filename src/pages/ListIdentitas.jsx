...
import { dummyIdentitas } from "../data/dummy";
...
const ListIdentitas = () => {
  ...
  const [identitas] = useState(dummyIdentitas);
  ...
  const filtered = identitas.filter(
    (id) => id.nama.toLowerCase().includes(search.toLowerCase()) ||
            id.nik.includes(search)
  );
        ...
        <div className="flex items-start justify-between flex-wrap gap-4 mb-6">
          <div>
            <h1 className={`text-xl font-bold tracking-wide ${
              ...
              Data <span style={{ color: currentColor }}>Identitas</span>
            </h1>
            <p className="text-xs mt-1" style={{ color: isDark ? "rgba(255,255,255,.35)" : "rgba(0,0,0,.5)" }}>
              Manajemen data identitas dan nomor-nomor penting pegawai
            </p>
          </div>
        ...
        >
          ...
          {!isLoading && (
            <div className="overflow-x-auto">
              <table className="w-full" style={{ minWidth: 1300, borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: isDark ? "rgba(56,139,255,.1)" : `rgba(${parseInt(currentColor.slice(1, 3), 16)},${parseInt(currentColor.slice(3, 5), 16)},${parseInt(currentColor.slice(5, 7), 16)},.08)`, borderBottom: `1px solid ${isDark ? "rgba(56,139,255,.2)" : `rgba(${parseInt(currentColor.slice(1, 3), 16)},${parseInt(currentColor.slice(3, 5), 16)},${parseInt(currentColor.slice(5, 7), 16)},.15)`}` }}>
                    {[
                      "No", "Nama", "NIK", "Nomor KK", "Nomor BPJS", "Nomor TASPEN", "Aksi",
                    ].map((h) => (
                      <th
                        key={h}
                  ...
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((id, i) => (
                      ...
                      >
                        ...
                        {/* Nomor KK */}
                        <td className="px-4 py-3 text-sm font-mono" style={{ color: isDark ? "rgba(220,235,255,.8)" : "rgba(0,0,0,.7)" }}>
                          {formatNomor(id.nomor_kk)}
                        </td>
                        {/* Nomor BPJS */}
                        <td className="px-4 py-3 text-sm font-mono" style={{ color: isDark ? "rgba(220,235,255,.8)" : "rgba(0,0,0,.7)" }}>
                          {formatNomor(id.nomor_bpjs)}
                        </td>
                        {/* Nomor TASPEN */}
                        <td className="px-4 py-3 text-sm font-mono" style={{ color: isDark ? "rgba(220,235,255,.8)" : "rgba(0,0,0,.7)" }}>
                          {formatNomor(id.nomor_taspen)}
                        </td>
                        {/* Aksi */}
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1.5">
                            {/* View */}
                            <button
                              onClick={() => navigate(`/identitas/${id.uuid}`)}
                              title="Lihat Detail"
                            ...
                            {/* Edit */}
                            <button
                              onClick={() => navigate(`/identitas/edit/${id.uuid}`)}
                              title="Edit"
                              className="w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-150 hover:-translate-y-0.5"
                              style={{
                                background: "transparent",
                            ...
                            {/* Delete */}
                            <button
                              onClick={() => setConfirmDelete(id.uuid)}
                              title="Hapus"
                              className="w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-150 hover:-translate-y-0.5"
                              style={{
                                background: "transparent",
        ...
        >
          ...
          >
            ...
            <p className="text-center text-sm mb-6" style={{ color: isDark ? "rgba(255,255,255,.4)" : "rgba(0,0,0,.5)" }}>
              Tindakan ini tidak dapat dibatalkan. Data identitas akan dihapus secara permanen.
            </p>
