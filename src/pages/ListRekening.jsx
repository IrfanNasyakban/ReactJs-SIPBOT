...
const ListRekening = () => {
  ...
  const filtered = rekening.filter(
    (r) =>
      (r.pegawai?.namaDenganGelar?.toLowerCase() ?? "").includes(
        search.toLowerCase(),
      ) ||
      (r.nomorRekGaji ?? "").includes(search) ||
      (r.namaBank?.toLowerCase() ?? "").includes(search.toLowerCase()),
  );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`${apiUrl}/rekening/${id}`, {
        headers: {
  ...
  const exportCSV = () => {
    const columns = [
      { key: "nama", label: "Nama" },
      { key: "nomorRekGaji", label: "Nomor Rek Gaji" },
      { key: "namaBank", label: "Nama Bank" },
      { key: "kantorCabang", label: "Kantor Cabang" },
    ];

    const headers = ["No", ...columns.map((c) => c.label)];

    const rows = rekening.map((r, i) => [
      i + 1,
      ...columns.map((c) => {
        const val =
          c.key === "nama"
        ...
        >
          ...
          {!isLoading && (
            <div className="overflow-x-auto">
              <table
                className="w-full"
                style={{ minWidth: 1200, borderCollapse: "collapse" }}
              >
                <thead>
                  <tr
                    style={{
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1.5">
                            {/* Edit */}
                            <button
                              onClick={() => navigate(`/rekening/edit/${r.id}`)}
                              title="Edit"
      ...
      {confirmDelete && (
        ...
        >
          ...
          >
            ...
            >
              Tindakan ini tidak dapat dibatalkan. Data rekening akan dihapus
              secara permanen.
            </p>
