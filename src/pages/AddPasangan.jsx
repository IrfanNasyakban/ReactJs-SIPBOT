...
const AddPasangan = () => {
  ...
  const getPegawai = async () => {
    try {
      ...
      const [pegawaiResponse, pasanganResponse] = await Promise.all([
        axios.get(`${apiUrl}/pegawai`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get(`${apiUrl}/pasangan`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);

      const existingIds = new Set(
        (pasanganResponse.data || []).map((item) => item.idPegawai),
      );
      ...
      <div className="relative z-10 p-7">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4 mb-8">
          <div className="flex-1">
            ...
            >
              Formulir Penambahan Data Pasangan Pegawai - Kantor Imigrasi Kelas
              II TPI Lhokseumawe
            </p>
          </div>
        </div>
