...
const STEPS = [
  ...
  { id: 6,  label: "Data Rekening",    status: "current" },
...
const AddRekeningNext = () => {
  ...
  useEffect(() => {
    if (namaBank !== "lainnya") setNamaLainnya("");
  }, [namaBank]);
  ...
  const saveRekening = async (e) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const response = await axios.post("http://localhost:5000/rekening", jsonData, {
        headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      });
      console.log("Response dari Server:", response);
      setLoading(false);
      navigate("/next/add/pendidikan", { state: { idPegawai, namaPegawai } });
    } catch (error) {
      setLoading(false);
  ...
  const onFocusSelect = (e) => {
    e.target.style.borderColor = currentColor;
    e.target.style.background = isDark ? `rgba(${rgb(currentColor)},.12)` : `rgba(${rgb(currentColor)},.05)`;
  };
                ...
                <div className="grid grid-cols-2 gap-6 mt-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onFocus={onFocusSelect} onBlur={onBlurSelect}
                    >
                      <option value="" style={{ color: "black" }}>-- Pilih Nama Bank --</option>
                      {BANK_OPTIONS.map((bank) => (
                        <option key={bank} value={bank} style={{ color: "black" }}>{bank}</option>
                      ))}
                      <option value="lainnya" style={{ color: "black" }}>Lainnya</option>
                    </select>
                  </div>

                  {/* Nama Bank Lainnya — conditional */}
                  {namaBank === "lainnya" && (
                    <div>
                      <label className="block text-sm font-semibold mb-2" style={labelStyle}>
                      ...
                      <input
                        name="namaLainnya" type="text"
                        required={namaBank === "lainnya"}
                        placeholder="Masukkan nama bank"
                        className="w-full px-4 py-2.5 rounded-xl text-sm transition-all duration-200"
                        style={inputBase} value={namaLainnya}
                  ...
                  <div>
                    ...
                    <input
                      name="nomorRekGaji" type="text" required
                      placeholder="Masukkan nomor rekening"
                      className="w-full px-4 py-2.5 rounded-xl text-sm transition-all duration-200"
                      style={inputBase} value={nomorRekGaji}
                      onChange={(e) => setNomorRekGaji(e.target.value.replace(/\D/g, ""))}
                      onFocus={onFocus} onBlur={onBlur}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      name="kantorCabang" type="text" required
                      placeholder="Contoh: KCP Lhokseumawe"
                      className="w-full px-4 py-2.5 rounded-xl text-sm transition-all duration-200"
                      style={inputBase} value={kantorCabang}
...
export default AddRekeningNext;