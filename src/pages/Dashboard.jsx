...
const Dashboard = () => {
  ...
  const sendMessageToAPI = async (userMessage) => {
    ...
    try {
      ...
      if (response.data.success) {
        if (!sessionId && response.data.data.conversationId)
          setSessionId(response.data.data.conversationId);
  ...
  const handleSendMessage = async () => {
    ...
    // Langsung tampilkan hasil — tanpa streaming
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <motion.div
          ...
          className={`mb-8 p-8 rounded-3xl border backdrop-blur-xl ${
            isDark
              ? "bg-gradient-to-br from-[#1a2332]/80 to-[#0f1419]/80 border-blue-500/20"
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className={`text-sm mt-2 flex items-center gap-2 ${isDark ? "text-blue-400" : "text-blue-600"}`}>
                ...
                {isTyping ? "Sedang memproses..." : "Online and ready to help"}
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          >
            ...
            <div className="h-px" style={{
              background: isDark
                ? "linear-gradient(90deg,transparent,rgba(56,139,255,.4),transparent)"
                : "linear-gradient(90deg,transparent,rgba(59,130,246,.2),transparent)",
            }} />
            ...
            <div className="h-[600px] overflow-y-auto p-6 md:p-8 space-y-5">
              {loading ? (
                <div className="flex justify-center items-center h-full">
                  <div className="text-center">
                    <div
                      className="animate-spin rounded-full h-14 w-14 border-2 mx-auto mb-4"
                  ...
                  {messages.map((message) => (
                    ...
                    >
                      ...
                      <div className={message.sender === "user" ? "max-w-[65%]" : "max-w-[85%] w-full"}>
                        ...
                        >
                          {message.sender === "user" ? (
                            <p style={{ fontSize: 15, lineHeight: 1.65, margin: 0, color: "#fff" }}>
                              {message.text}
                            </p>
                          ) : (
              ...
              {/* Thinking indicator — hanya dots, bukan running text */}
              {isTyping && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
              ...
              <div className="flex gap-4 items-end">
                <div className="flex-grow relative">
                  <input
                    ...
                    disabled={isTyping}
                    placeholder={isTyping ? "Menunggu SIPBOT..." : "Tanyakan informasi data pegawai..."}
                    className={`w-full px-5 py-3 rounded-2xl border-2 transition-all text-sm md:text-base outline-none ${
                ...
                <motion.button
                  ...
                  className={`p-3 rounded-full transition-all flex-shrink-0 text-white ${
