...
const ListFisik = () => {
  ...
  const exportCSV = () => {
    const columns = [
      ...
      { key: "jenisRambut", label: "Jenis Rambut" },
      { key: "warnaRambut", label: "Warna Rambut" },
      { key: "bentukWajah", label: "Bentuk Wajah" },
      { key: "warnaKulit", label: "Warna Kulit" },
      { key: "ciriKhusus", label: "Ciri Khusus" },
    ];

    const headers = ["No", ...columns.map((c) => c.label)];

    const rows = fisik.map((f, i) => [
      i + 1,
      ...columns.map((c) => {
        const val =
          c.key === "nama"
      ...
      {confirmDelete && (
        ...
        >
          ...
          >
            ...
            >
              Tindakan ini tidak dapat dibatalkan. Data fisik akan dihapus
              secara permanen.
            </p>
